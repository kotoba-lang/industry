...
(deftest maturity-tier
  ...
  (testing "maturity-summary counts tiers"
    (let [m (industry/maturity-summary)]
      ...
      ;; this promotion's own +1 for cloud-itonami-isic-2396, recomputed
      ;; live via `(kotoba.industry/maturity-summary)` against a freshly
      ;; re-fetched origin/main tip immediately before this PUT (not
      ;; assumed), folding in any further concurrent sibling promotions
      ;; not individually re-narrated here; this is the final actor of
      ;; Wave 3 -- only the two deliberately-scoped-out sensitive classes
      ;; (2520 weapons/ammunition, 3040 military fighting vehicles) remain
      ;; unimplemented in the entire wave.
      (testing "cloud-itonami-isic-2396 (Cutting, shaping and finishing of stone, fresh scaffold -- no prior repository at either the stale gftdcojp/cloud-itonami-C2396 placeholder or the real cloud-itonami org [gh api 404 confirmed]; identity ({:id \"2396\" :name \"Cutting, shaping and finishing of stone\"}) independently verified against a fresh clone before any work began, per this fleet's ID/name-mismatch caution -- 2396 covers dimension-stone FABRICATION (quarried-block intake -> sawing -> polishing -> edge-profiling/CNC shaping -> inspection), a downstream process distinct from stone quarrying/extraction itself [ISIC 0810, out of scope -- this actor never proposes anything about a quarry face, blast plan or block extraction]; StoneAdvisor ⊣ Dimension Stone Plant Operations Governor plant-operations-coordination actor mirroring cloud-itonami-isic-2393's [Manufacture of other porcelain and ceramic products] verified module shape module-for-module (stonemfg.* in place of porcelainmfg.*) -- sawing-line/polishing-line equipment registration in place of forming-line/kiln-line equipment, and granite-slab/marble-slab/limestone-slab/sandstone-slab/monument/countertop/dimension-stone-tile/cut-stone-veneer production-batch (product-type/slab-thickness-tolerance-mm/polish-gloss-level) vocabulary in place of porcelain-tableware/bone-china-tableware/stoneware-tableware/earthenware-tableware/decorative-ceramic/art-pottery/technical-ceramic/ceramic-insulator and glaze-defect-rate-percent/chip-resistance-newtons; :flag-safety-concern (crystalline-silica-dust exposure, saw-blade/wire-saw kickback and pinch-point hazard, heavy-slab handling/crush hazard) always escalates regardless of confidence, matching 2393's own safety-concern-escalation invariant; the proposal-effect allowlist plus a permanent equipment-actuate block (`:actuate-sawing-polishing-line? true`) structurally prevent any direct sawing-line/polishing-line-equipment control, with no human-approval override path; eleven concrete governor checks elaborate the same four HARD invariants as 2393 (propose-only effect, closed op allowlist, closed proposal-effect allowlist, permanent sawing/polishing-line-actuate block, independent equipment/batch verification/registration before any action, independent shipment-weight recompute, double-schedule guard, product-type/slab-thickness-tolerance/polish-gloss-level validation); :schedule-maintenance/:flag-safety-concern/:coordinate-shipment are NEVER in any phase's :auto set, only :log-production-batch may auto-commit at phase 3 when clean; fully portable .cljc with no JVM-only interop; 75 tests / 210 assertions green, independently re-verified against a fresh clone; registry.edn's own \"2396\" -> :implemented change (plus :repo/:business-id de-placeholdered from gftdcojp/cloud-itonami-C2396 to the real cloud-itonami org target cloud-itonami/cloud-itonami-isic-2396) landed via a Contents-API single-file PUT (sha-checked optimistic concurrency, freshly re-fetched content immediately before the PUT per this fleet's hot-contention discipline), exact-block edit only, diff-verified single-block change (:repo/:business-id/:maturity only); superproject ADR-2613200000) is also :implemented"
        (is (= :implemented (industry/maturity "2396"))))
      (is (= 369 (:implemented m))))))
